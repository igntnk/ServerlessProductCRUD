...
func UpdateProduct(rw http.ResponseWriter, req *http.Request) {
	...
	result, err := db.Query().Query(timeCtx, fmt.Sprintf("UPSERT INTO products ( `id`, `name` ) VALUES ( %v, '%v') returning id, name, created_at;", input.Id, input.Name))
	if err != nil {
	...
	err = row.Scan(&product.Id, &product.Name, &product.CreatedAt)
	if err != nil {
